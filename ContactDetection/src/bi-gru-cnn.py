...
def bi_gru_cnn(embedding_matrix):
    ...
    x1 = Conv1D(32, kernel_size=3, padding="valid", kernel_initializer="glorot_uniform")(x1)

    x2 = SpatialDropout1D(0.5)(x)
    x2 = Bidirectional(GRU(128, return_sequences=True))(x2)
    x2 = Conv1D(64, kernel_size=2, padding="valid", kernel_initializer="glorot_uniform")(x2)

    x3 = SpatialDropout1D(0.5)(x)
    x3 = Bidirectional(GRU(32, return_sequences=True))(x3)
    x3 = Conv1D(16, kernel_size=4, padding="valid", kernel_initializer="glorot_uniform")(x3)
...
with utils.timer('Train'):
    for s in range(config.train_times):
        ...
        for fold, (train_index, valid_index) in enumerate(skf.split(X, y)):
            ...
            model = bi_gru_cnn(embedding_matrix)
